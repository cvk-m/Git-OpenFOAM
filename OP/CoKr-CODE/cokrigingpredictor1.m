function metric=cokrigingpredictor1(x)
...
cc=ones(nc,1);
for i=1:nc
	cc(i)=rho*ModelInfo1.SigmaSqrc*exp(-sum(thetac.*abs(Xc(i,:)-x).^p));
end
cd=ones(ne,1);
for i=1:ne
	cd(i)=rho^2*ModelInfo1.SigmaSqrc*exp(-sum(thetac.*abs(Xe(i,:)-x).^p))+ModelInfo1.SigmaSqrd*exp(-sum(thetad.*abs(Xe(i,:)-x).^p));
end
c=[cc;cd];
f=ModelInfo1.mu+c'*(ModelInfo1.UC\(ModelInfo1.UC'\(y-one.*ModelInfo1.mu))); 
if strcmp(ModelInfo1.Option,'Pred')==0
    SSqr=rho^2*ModelInfo1.SigmaSqrc+ModelInfo1.SigmaSqrd-c'*(ModelInfo1.UC\(ModelInfo1.UC'\c));
    s=abs(SSqr)^0.5;
    if strcmp(ModelInfo1.Option,'RMSE')==0
            yBest=min(ye);
        if strcmp(ModelInfo1.Option,'NegProbImp')==1
...
if strcmp(ModelInfo1.Option,'Pred')==1
metric=f;
elseif strcmp(ModelInfo1.Option,'RMSE')==1
metric=s;
elseif strcmp(ModelInfo1.Option,'NegLogExpImp')==1
metric=-ExpImp;
elseif strcmp(ModelInfo1.Option,'NegProbImp')==1
metric=-ProbImp;
end
