clc
clear all
%p=[0.0155 0.296632 0.060015 -0.4515 0.296632 -0.06055 0.453 0 0.001260 0 7.36]; %%NACA 0012
p=[0.0216 0.3445 0.07912 -0.6448 0.17 -0.033797 0.6748 0 0 0 0];  %%NACA 2411;
%p=[0.0216 0.3445 0.07912 -0.6448 0.17 -0.033797 0.6748 0 0 0 0]
Nfoil=200;% number of point to built airfoil base shape
Nf=Nfoil+1;
dbeta=pi/Nfoil;
LE=0.1;  %LEADING EDGE MODIFICATION  LENGHT  
fLl = 4; %frequency

TP=0.9;

teta =10; %randi([2 50],1,1)twist
DA=4;
SW=2;

TE=0.1;  %TRAILING EDGE MODIFICATION  LENGHT  
fT=10;
...
WYA=10;
fL = fT;
ff=[LE,fLl,TP,teta,DA,SW,TE,fT,Rw,wtp,WXA,WYA];
fWe=[];
for i=1:12
    if i==12
...
yufoil=a(1)*x.^.5+a(2)*x.^(1.5)+a(3)*x.^(2.5)+a(4)*x.^(3.5)+a(5)*x.^(4.5)+a(6)*x.^(5.5);
ylfoil=a(7)*x.^.5+a(8)*x.^(1.5)+a(9)*x.^(2.5)+a(10)*x.^(3.5)+a(11)*x.^(4.5)+a(12)*x.^(5.5);
...
dbl=pi/(Yl*2);
    for g=1:(Yl-1)
    rl(g)=((1-cos(beta)))*(x(Yl)+stb)-stb;
    beta=beta+dbl;
    end
rl((Yl-1))=(rl((Yl-1))+x((Yl-1)))/2;
Lxl=[rl,x(1,(Yl):Nfoil+1)];
LXl=cat(2,LXl,Lxl);
end
...
%dbl=pi/((Nfoil*0.25)*2)
%for i=0:dbl:pi/2
  % su(g)=((1-cos(beta))*(x(Nfoil+1)-sst-(x(Nfoil*0.75))))+(x(Nfoil*0.75));
   
  % beta=beta+dbl;
 %   g=g+1;
%end
%xsu=[x(1,1:Nfoil*0.7),su]
%yxx=a(1)*su.^.5+a(2)*su.^(1.5)+a(3)*su.^(2.5)+a(4)*su.^(3.5)+a(5)*su.^(4.5)+a(6)*su.^(5.5);
%yuxx=[yufoil(1,1:Nfoil*0.8),yxx]
%PP=polyfit(x(1,1:169),yufoil(1,1:169),2)


%for i=1:1:51
%YXX (i)= (PP(1)*su(i)^2+PP(2)*su(i)^1+PP(3))
%end
%plot(su,YXX,'*')
%hold on
%plot(x,yufoil,'*')
...
%if TEP(i)==0
  %  TXl(a)=TXu(a);
    TXl=TXu;
%elseif LXl(a)>((0.5+LE/2)-TEP(i))
  %TXl(a)=TXu(a);
    %TXl(a)=((0.5+LE/2)-TEP(i));
%else
 %   TXl(a)=TXu(a);
%end
 %    end
...
%%PTXu=(find(LXu(((i-1)*(Nfoil+1)+1):i*(Nfoil+1))>0.9))
%for a=PTXu:Nfoil*i
  %  TXu(a)=(1-TEP(i))
%end
%end
...
taf=Nf-(round((Nf)*TE*2.5)+1);  %
Ntaf=(round((Nf)*TE*2.5)+1);
 ptafu=[];
 ptafl=[];
for i=1:Na
...
%ntaf ile son çizgi arasý üst yüzey bölme nokta+ son çizgi ayýran nokta  k1
%k2
...
%fprintf(fid,'Transfinite Curve {%i:%i}= 100 Using Progression 1; \r\n',s5+1,s5+fL); %ntaf ile trailing arasý ayýran çizgiler up
%fprintf(fid,'Transfinite Curve {%i:%i}= 100 Using Progression 1; \r\n',s6+1,s6+fL); %ntaf ile trailing arasý ayýran çizgiler low
...
%fprintf(fid,'Transfinite Curve {%i:%i}= 20 Using Progression 1 ;\r\n',s3+1,s3+fL);% ntaf çizgi uýp edge çizgi
